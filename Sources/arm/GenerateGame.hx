...
class GenerateGame extends iron.Trait {
	...
	public function new() {
		...
		notifyOnInit(function() {
			...
			var index = 0;
			var distance = 0;
			var temp:Array<{i:Int, x:Int, y:Int, z:Int, t:Int, v:Float, p:Int, o:Object, n:Array<Int>, out:Int, outO:Object, ringO:Object, inI:Array<Int>}> = [];
				...
				for (r in -radius...radius+1) {
					for (s in -radius...radius+1) {
						if (q+r+s == 0) { //Filters to only applicable hexagons
							...
							temp = [{i:index, x:q, y:r, z:s, t:Std.random(20), v:Std.int(1750*(Math.random()+0.5)), p:0, o:null, n:null, out:null, outO:null, ringO:null, inI:[]}];
							data.push(temp);
							index += 1;
						}
					}
