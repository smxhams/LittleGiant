...
void frag_main()
{
    ...
    {
        _390 = octahedronWrap(g0.xy);
    }
    n = float3(_390.x, _390.y, n.z);
    n = normalize(n);
    ...
    float3 _502 = fragColor.xyz * ssaotex.SampleLevel(_ssaotex_sampler, texCoord, 0.0f).x;
    fragColor = float4(_502.x, _502.y, _502.z, fragColor.w);
    if (g0.w == 1.0f)
    {
        float3 _514 = fragColor.xyz + g1.xyz;
        fragColor = float4(_514.x, _514.y, _514.z, fragColor.w);
        albedo = 0.0f.xxx;
    }
    ...
    float3 _559 = fragColor.xyz + ((sdirect * svisibility) * sunCol);
    fragColor = float4(_559.x, _559.y, _559.z, fragColor.w);
    fragColor.w = 1.0f;
}
